...
// AndySchmitz : 06/29/2005 - modified for ActiveLock <andy.schmitz@gmail.com>
//
// $Id: gateway.php.txt,v 1.2 2005/06/10 05:26:49 geoid Exp $
// Copyright 2003-2005. BitPass, Inc. All rights reserved.
//
/*---------------------------------------------------------------------------*/
...
// publisher ID - Change this to your ID.
//  It is your 'handle' on https://www.bitpass.com/earn/account/.
$pubid = '00000B11';

// phpALUGen directory - Set this to the directory of phpALUGen.
//  THIS MUST BE SET CORRECTLY FOR ACTIVELOCK TO WORK
$phpALUGen_include_dir = '..';

// phpALUGen Key Template - Set this to a template to use when a key is generated
/*  Use either <?=$licKeySplit?> or <?=$licKey?> in your template to display the key */
//  If this is not set, the gateway will output a simple screen giving your user the key.
//  Set this to either an absolute path or a path relative to this script.
$phpALUGen_template = '';

// base directory which contains the premium content
//  You will only need to set this if you plan on providing non-ActiveLock content
//  Set this to the path relative to the 'Base URL' you gave to BitPass, but you 
//  may not be able to use '..' to ascend directories.
$base = 'interface/paid/';

// name of the index file, which usually is one of
// index.html, index.htm, index.cgi, index.asp or index.php (optional)
$index = 'index.html:index.htm:index.php:default.html:default.htm';

// pass key phrase (change this to something base64-encoded)
$secret = 'c2VjcmV0MTM1MjcyODYzODAw';
...
header('X-BITPASS-GW-VER: PHP $Revision: 1.2 $', FALSE);
...
// test whether $base is a real directory (don't do that - ActiveLock means we need a basedir)
//if (!is_dir($base)) {
...
if ($res == 1) {
	if (isSet($_GET['_bp_save_'])) {
		...
		if ($GW_SCRIPT) {
			// Check to see if it's an ActiveLock key request
			$filename = basename($GW_SCRIPT);
			if (substr($filename, 0, 5) == 'ALLK-') {
				$product = base64_decode(substr($filename, 5, -4));
				
				// Include the phpALUGen library
				$curCwd = getcwd();
				chdir($phpALUGen_include_dir);
				require_once 'phpALUGen.php';
				chdir($curCwd);
				
				// Set up the product
				$prodParts = explode(' - ', $product);
				
				// Set up the license
				$license = new phpALUGen_License;
				$license->ProductName = $prodParts[0];
				$license->ProductVer = $prodParts[1];
				$license->RegisteredDate = date('Y/m/d');
				$license->LicenseClass = 'Single';
				$license->LicenseType = 2; // Permanent
				$license->Expiration = '9999/12/15'; // Dummy date - shouldn't matter
				$license->MaxCount = '0';
				
				$phpALUGen = new phpALUGen;
				$libKey = $phpALUGen->genKey($license, $ic);
				$libKeySplit = chunk_split($libKey, 80);
				
				// Check to see if we have a template
				if ($phpALUGen_template) {
					// Include the supplied template
					include $phpALUGen_template;
				} else {
					// Print it out with a default template (it looks bad - use a template)
					echo '<html><head><title>ActiveLock Liberation Key</title></head>';
					echo '<body><h1>Here is your Liberation Key:</h1><pre>'.$libKeySplit.'</pre>';
					echo '<br /><br />Simply enter this code into your application and your program should be permanently registered.';
					echo '</body></html>';
				}
				
				// We've done everything we are supposed to, now we exit
				exit;
			}
...
//
/* bcpowmod is provided by BCMath, don't redefine it here
...
function bc_add($a, $b) {
    ...
    return external(array_merge($x_ar, $y_ar, $car));
}
